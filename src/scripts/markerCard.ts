...
import type { MarkerViewport } from './markerViewport.ts';

export function defaultConfig(name: string): MarkerConfig {
	return {
		markerName: name,
...
function quatToEulerDeg(q: Partial<Quat>): Vec3 {
	...
	if (outOfRange) return { x: round(xq), y: round(yq), z: round(zq) };
...
function eulerDegToQuat(eDeg: Partial<Vec3>): Quat {
	const r = Math.PI / 180;
...
interface VecRowOptions {
	readOnlyKeys?: string[];
	onAfterEdit?: ((key: string, inputs: Record<string, HTMLInputElement>) => void) | null;
}
...
function makeBtn(iconName: string, label: string, className = ''): HTMLButtonElement {
...
export interface MarkerCardCallbacks {
	isDirty: boolean;
	onSave: (cfg: MarkerConfig, btn: HTMLButtonElement, card: HTMLElement) => void;
	onDelete: (name: string) => void;
	onRename: (cfg: MarkerConfig, newName: string, card: HTMLElement, nameWrap: HTMLElement) => Promise<void>;
	onDirtyChange: (name: string) => void;
}

export function createMarkerCard(cfg: MarkerConfig, { isDirty, onSave, onDelete, onRename, onDirtyChange }: MarkerCardCallbacks): HTMLElement {
	{
		const q = (cfg.rotationOffset as Partial<Quat>) || { x: 0, y: 0, z: 0, w: 1 };
		if (
	...
	let viewport: MarkerViewport | null = null;
	...
	const ensureViewport = () => {
		...
		import('./markerViewport.ts')
			.then((mod) => {
				viewport = mod.createMarkerViewport(viewportEl, cfg);
				viewport.setFlipped(flipped);
			})
			.catch((err) => { console.error('Viewport load failed:', err); })
			.finally(() => { viewportLoading = false; });
	};
	...
	renameBtn.addEventListener('click', () => {
		...
		const cancel = () => { row.replaceWith(nameWrap); };
		const confirm = async () => {
	...
	body.appendChild(vecRow('Position offset', 'move', cfg.positionOffset as Record<string, number>, ['x', 'y', 'z'], markDirty));

	const eulerState = quatToEulerDeg(cfg.rotationOffset as Partial<Quat>);
	body.appendChild(vecRow(
		...
		['x', 'y', 'z'],
		() => {
			Object.assign(cfg.rotationOffset as object, eulerDegToQuat(eulerState));
			markDirty();
		},
	));

	body.appendChild(vecRow('Scale multiplier', 'scale', cfg.scaleMultiplier as Record<string, number>, ['x', 'y', 'z'], markDirty));
